...
ALL_TIME T3
ON 1=1
AND T2.DATA_DATE BETWEEN T3.START_TIME AND T3.END_TIME
WHERE 1=1
AND T3.CATEGORY = '${P_TIME_UNIT}'
${IF(LEN(P_TIME_UNIT)==0,"","AND T2.DATA_DATE BETWEEN TO_DATE('"+P_TIME_START+"','yyyy-mm-dd') AND TO_DATE('"+P_TIME_END+"','yyyy-mm-dd')")}
GROUP BY
T1.DEPTID,
DEPT_NAME,
SCORE
...
SELECT
        ...
        SUM(T2.SCORE) + SUM(T4.OTHER_SCORE) AS SCORE
...
--前兩年同期
AND T2.DATA_DATE BETWEEN ADD_MONTHS(T3.START_TIME,-12) AND ADD_MONTHS(T3.END_TIME,-12)
WHERE 1=1
AND T1.DEPT_CATEGORY IS NOT NULL
...
ON T3.CATEGORY = '${P_TIME_UNIT}'
--前兩年同期
AND T2.DATA_DATE BETWEEN ADD_MONTHS(T3.START_TIME,-24) AND ADD_MONTHS(T3.END_TIME,-24)
WHERE
1=1 
AND T1.DEPT_CATEGORY IS NOT NULL
AND T1.DEPT_LEVEL = '分行'
